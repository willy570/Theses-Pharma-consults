...
<main role="main" class="container mt-5">
  <div class="d-flex p-3 my-3 text-white-50 bg-white rounded shadow-sm">
    <div class="lh-100">
     <span class="badge badge-info">{{$u->denomination}}</span>
     <span class="badge badge-info">{{$d->libelle}}</span>
     <span class="badge badge-info">{{$ds->libelle}}</span>
    </div>
  </div>
 ...
 <div class="row">
    ...
    <div class="col-md-12 blog-main">
      <div class="col p-3 bg-white rounded shadow-sm">
        <h6 class="border-bottom border-gray pb-2 mb-2">
          @if(count($results) == 0)
            {{$results->count()}}&nbsp;document trouvé
          @else
          {{count($results)}}&nbsp;documents trouvés
          @endif
        </h6>
      @if($results->isEmpty())
      <p class="alert alert-info p-3">Aucun document trouvé!</p>
      @else
        ...
        <div class="media text-muted pt-3">
          ...
          <p class="media-body pb-3 mb-0 small lh-125 border-bottom border-gray">
            <strong class="d-block text-gray-dark"><a href="{{route('theses.details', $r->url)}}">{{$r->titre}}</a></strong><br>
      ...
      @endif
        ...
        <small class="d-block text-right mt-3">
          <a href="{{route('theses.all')}}">Retourner</a>
        </small>
  </div>
